...
import { BytesLike, keccak256, solidityKeccak256, parseEther } from 'ethers/lib/utils'
...
export const executeSendSuccess = async (
  wallets: Wallet[],
  owners: string[],
  threshold: number,
  ...
  await multisig.execute(sigs.sigV, sigs.sigR, sigs.sigS, randomAddr, value, EMPTY_DATA, executor, 21000, {
    from: msgSender,
  })
  ...
  await multisig.execute(sigs.sigV, sigs.sigR, sigs.sigS, randomAddr, value, EMPTY_DATA, ZEROADDR, 21000, {
    from: msgSender,
  })
...
export const executeSendFailure = async (
  wallets: Wallet[],
  owners: string[],
  threshold: number,
  ...
  await expect(
    multisig.execute(sigs.sigV, sigs.sigR, sigs.sigS, randomAddr, value, EMPTY_DATA, executor, 21000, {
      from: msgSender,
    })
